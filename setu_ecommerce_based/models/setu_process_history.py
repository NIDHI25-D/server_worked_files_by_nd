...
class SetuProcessHistory(models.Model):
    ...
    history_perform = fields.Selection(
        selection=[('import', 'Import'), ('export', 'Export'), ('cancel', 'Cancel'),
                   ('update', 'Update'), ('refund', 'Refund'), ('conn', 'Connection'),
                   ('prepare_create', 'Prepare Create'), ('prepare_update', 'Prepare Update'),
                   ('record_fetch', 'Fetch')],
        string="Operation Perform")
    ...
    process_history_line_ids = fields.One2many(
        comodel_name="setu.process.history.line", 
        inverse_name="process_history_id", string="Process History Line")
    ...
    def create(self, vals_list):
        ...
        @name : Kamlesh Singh
        @date : 28/10/2024
        @purpose : This method will use to generate sequence for process history
        :param vals_list: 
