...
#        if(self.structure_id.type_id.default_struct_id == self.structure_id):
        ...
        for employee in self.env['hr.employee'].browse(data['employee_ids']):
            ...
            holiday_inc = self.env['hr.leave'].search([('request_date_from','>=', fields.Date.from_string(date_from)), ('request_date_from', '<=', fields.Date.from_string(date_to)), 
                                                       ('holiday_status_id.code', '=', 'INC_EG'), ('state', '=', 'validate')], limit=1)
            if holiday_inc:
                res.update({'dias_pagar_incapacidad': holiday_inc.dias_pagar,})
                holiday_inc.dias_pagar = 0
