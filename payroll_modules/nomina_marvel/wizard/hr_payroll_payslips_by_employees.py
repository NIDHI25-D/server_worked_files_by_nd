...
class HrPayslipEmployeesExt(models.TransientModel):
    ...
    def compute_sheet(self):
        ...
        if active_id:
            [run_data] = self.env['hr.payslip.run'].browse(active_id).read(['incidence_start', 'incidence_end', 'date_start', 'date_end'])
        else:
            return
        from_date = run_data.get('incidence_start')
        to_date = run_data.get('incidence_end')
