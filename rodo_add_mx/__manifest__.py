# -*- coding: utf-8 -*-

{
    'name': 'RODO ADD MX',
    'version': '18.0.0.0',
    'description': ''' 
                    Descarga los CFDI directo a Odoo por medio del API de TuOdoo México
                    para su procesamiento y administracion.

                    Se necesita de la libreria de python xmltodict
                    - sudo pip3 install xmltodict

                    Puede adquirir liencia para este módulo en +52 5661443104
                    ''',
    'category': 'Accounting',
    'author': 'TuOdoo MEXICO',
    'website': 'www.tuodoomexico.com',
    'depends':      [
                    'account','l10n_mx_edi','sale_management','purchase','account_accountant'
                    ],
    'data':         [
                    'security/ir.model.access.csv',
                    'security/l10n_mx_edi_esignature.xml',
                    'security/security.xml',
                    'data/cron_data.xml',

                    'views/ir_attachment_view.xml',
                    'views/res_config_settings_view.xml',
                    #'views/templates.xml',
                    'views/esignature_view.xml',
                    'views/account_move_view.xml',
                    'views/account_tax_view.xml',
                    'views/solicitud_ws.xml',

                    'wizard/cfdi_invoice.xml',
                    'wizard/import_invoice_process_message.xml',
                    'wizard/reconcile_vendor_cfdi_xml_bill.xml',
                    'wizard/xml_invoice_reconcile_view.xml',
                    'wizard/descarga_x_dia_wizard.xml',
                    'wizard/attach_xmls_wizard_view.xml',
                    'report/report_facturas_de_clientes_or_proveedores.xml',
                    'report/payment_report_from_xml.xml',
                    ],
    'assets': {
        'web.assets_backend': [
           'rodo_add_mx/static/src/js/**/*',
           'rodo_add_mx/static/src/css/**/*',
           'rodo_add_mx/static/src/xml/*.xml',
        ],
    },
    'application': False,
    'installable': True,
    'license': 'OPL-1',
}
