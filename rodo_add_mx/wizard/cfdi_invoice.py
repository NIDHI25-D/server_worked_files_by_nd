...
class CfdiInvoiceAttachment(models.TransientModel):
    ...
    supplier_account_analytic_id = fields.Many2one('account.analytic.account', string='Analytic Account')
    ...
    def import_xml_file(self):
        ...
        if model == 'ir.attachment' and active_ids:
            ...
            create_vendor_bill_and_credit_notes_only = ctx.get('create_vendor_bill_and_credit_notes_only', False)
            ...
            for attachment in attachments:
                ...
                try:
                    ...
                    if create_so_po:
                        ...
                        if attachment.cfdi_type == 'I':
                            if create_vendor_bill_and_credit_notes_only:
                                raise UserError('Please select only Facturas de proveedor and Notas de crédito proveedor in Tipo de comprobante because from here only vendor bills and vendor credit note are created.')
                            else:
                            ...
                            res = self.import_supplier_invoice(attachment.datas, self.supplier_journal_id,
                                                               self.supplier_account_id, self.supplier_account_analytic_id)
                        elif attachment.cfdi_type == 'E':
                            if create_vendor_bill_and_credit_notes_only:
                                raise UserError(
                                    'Please select only Facturas de proveedor and Notas de crédito proveedor in Tipo de comprobante because from here only vendor bills and vendor credit note are created.')
                            else:
    ...
    def import_supplier_invoice(self, file_content, journal=False, account=False, analytic_account=False):
        ...
        for line in invoice_line_data:
            ...
            taxes = line.get('Impuestos', {}).get('Traslados', {}).get('Traslado')
            if data.get('Comprobante', {}).get('Complemento', {}).get('implocal:ImpuestosLocales', {}).get(
                    'implocal:TrasladosLocales', {}).get('@ImpLocTrasladado') and data.get('Comprobante', {}).get(
                    'Complemento', {}).get('implocal:ImpuestosLocales', {}).get('implocal:TrasladosLocales', {}).get(
                    '@TasadeTraslado'):
                taxes.update(data.get('Comprobante', {}).get('Complemento', {}).get('implocal:ImpuestosLocales').get(
    ...
    @api.model
    def import_supplier_credit_note(self, file_content, journal=False, account=False, analytic_account=False):
        file_content = base64.b64decode(file_content)
        file_content = file_content.replace(b'cfdi:', b'')
        file_content = file_content.replace(b'tfd:', b'')
        try:
    ...
    def get_tax_from_codes(self, taxes,tax_type,no_imp_tras):
        ...
        if taxes:
            ...
            for tax in taxes:
                ...
                print("tax.get('@Impuesto')", tax.get('@Impuesto'))
                print("tax_type", tax_type)
                print("ttax.get('@TipoFactor')", tax.get('@TipoFactor'))
                print("tasa", tasa)
                print("self.env.company", self.env.company.name)

                domain = [('type_tax_use', '=', tax_type), ('l10n_mx_factor_type', '=', tax.get('@TipoFactor')),
                          ('company_id', '=', self.env.company.id)]
                tax_exist = tax_obj.search(domain + [('impuesto', '=', tax.get('@Impuesto')), ('amount', '=', tasa)],
                                           limit=1)
                if tax.get('@TasadeTraslado'):
                    another_tax = tax.get('@TasadeTraslado')
                    another_tax_exist = tax_obj.search(domain + [('amount', '=', another_tax)],
                                                       limit=1) or tax_obj.search(
                        domain + [('amount', '=', str(float(another_tax) * 100))], limit=1)
                else:
                    another_tax_exist = False

                if not tax_exist and not another_tax_exist:
                    raise UserError("La factura contiene impuestos que no han sido configurados. Por favor configure los impuestos primero")
                if tax_exist:
                    tax_ids.append(tax_exist.id)
                if another_tax_exist:
                    tax_ids.append(another_tax_exist.id)
                k = k+1
        return tax_ids
