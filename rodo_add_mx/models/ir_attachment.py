...
class IrAttachment(models.Model):
    ...
    solicitud_ws_id = fields.Many2one(
        string='Solicitud WS TOM',
        comodel_name='solicitud.ws',
    )

    fecha = fields.Date(
        string='Fecha de solicitud',        
        related='solicitud_ws_id.fecha',
        readonly=True,
        store=True
        
    )

    state_creado_en_odoo = fields.Selection([
        ('done', ''),
        ('blocked', '')], string='Creado en odoo',
        copy=False, default='done', compute='_compute_state_creado_en_odoo')
    
    @api.depends('creado_en_odoo')
    def _compute_state_creado_en_odoo(self):
        for rec in self:
            if rec.creado_en_odoo:
                rec.state_creado_en_odoo = 'done'
            else:
                rec.state_creado_en_odoo = 'blocked'
