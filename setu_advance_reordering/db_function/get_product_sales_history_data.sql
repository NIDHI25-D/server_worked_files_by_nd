...
CREATE OR REPLACE FUNCTION public.get_product_sales_history_data(
    ...
    IN start_date date,
    IN end_date date)
  RETURNS TABLE(company_id integer, product_id integer, product_category_id integer, warehouse_id integer, sales_qty numeric, total_orders numeric, ads numeric, max_daily_sale numeric, min_daily_sale numeric, t_sales_qty numeric, t_sales_return_qty numeric, sales_day numeric) AS
$BODY$
    DECLARE
        ...
        ELSE
         Return Query
            Select
                ...
                sum(sd.total_orders) as total_orders,
                case when (sum(sd.total_sales) - sum(sd.total_return_qty)) <= 0 then 0 else
                round((sum(sd.total_sales) - sum(sd.total_return_qty)) /sd.sales_days,2) end as ads,
                max(sd.total_sales) max_daily_sale,
                --min(sd.total_sales) min_daily_sale,
                MIN(NULLIF(sd.total_sales, 0)) min_daily_sale,
                sum(sd.total_sales) as t_sales_qty,
                sum(sd.total_return_qty) as t_sales_return_qty,
                sd.sales_days as sales_day
            From (Select
                    cmp_id,
                    p_id,
                    categ_id,
                    ...
                    count(T.order_date) OVER (PARTITION BY cmp_id, p_id, categ_id, wh_id)::numeric AS sales_days
                  ...
                  From(Select
                            ...
                            foo.pos_order_id,
                            foo.order_date,
                            count(foo.order_date) OVER (PARTITION BY cmp_id, p_id, categ_id, wh_id) AS sales_days
                       From (Select
                                move.company_id as cmp_id,
                                move.product_id as p_id,
                                tmpl.categ_id,
                ...
                group by cmp_id, p_id, categ_id, wh_id ,sales_days;
           END IF;
