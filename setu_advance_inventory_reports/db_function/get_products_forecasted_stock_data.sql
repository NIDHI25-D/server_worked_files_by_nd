...
BEGIN
    Return Query
    Select
        T.company_id, cmp.name as company_name, T.product_id,
        concat('['||(prod.default_code||'')||']'||' '||(tmpl.name ->>'en_US'))::character varying as product_name,
    ...
    (
        ...
        from
            stock_quant quant
                ...
                Left Join stock_warehouse ware ON loc.parent_path::text ~~ concat('%/', ware.view_location_id, '/%')
                                                           -- change tmpl.type = 'product' to tmpl.is_storable = 'True'
        Where loc.usage = 'internal' and prod.active = True and tmpl.active = True and tmpl.is_storable = 'True'
        ...
        Where
            source.usage = 'supplier' and dest_location.usage = 'internal' and move.state not in ('draft','done','cancel') and
                                                          -- change tmpl.type = 'product' to tmpl.is_storable = 'True'
            prod.active = True and tmpl.active = True and tmpl.is_storable = 'True'
            --company dynamic condition
            and 1 = case when array_length(company_ids,1) >= 1 then
