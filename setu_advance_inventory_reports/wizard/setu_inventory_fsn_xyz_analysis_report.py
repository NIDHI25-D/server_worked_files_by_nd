...
class SetuInventoryFSNXYZAnalysisReport(models.TransientModel):
    ...
    product_ids = fields.Many2many("product.product", 'fsn_xyz_ana_repo_product_rel',
                                   'fsn_xyz_ana_report_id', 'product_id', string="Products")

    warehouse_ids = fields.Many2many("stock.warehouse", 'fsn_xyz_ana_repo_warehouse_rel',
                                     'fsn_xyz_ana_report_id', 'warehouse_id', string="Warehouses")
    warehouses_ids = fields.Many2many("stock.warehouse", 'fsn_xyz_analysis_repo_warehouse_rel',
                                      'fsn_xyz_analysis_report_id', 'warehouses_id',
                                      string="Warehouses", compute="_compute_warehouses_ids")

    stock_movement_type = fields.Selection([('all', 'All'),
                                            ('fast', 'Fast Moving'),
                                            ('slow', 'Slow Moving'),
                                            ('non', 'Non Moving')], string="FSN Classification", default="all")
    inventory_analysis_type = fields.Selection([('all', 'All'),
                                                ('high_stock', 'X Class'),
                                                ('medium_stock', 'Y Class'),
                                                ('low_stock', 'Z Class')], string="XYZ Classification", default="all")
    ...
    def download_report(self):
        ...
        return {
            ...
            'url': '/web/binary/setu_air_download_document?model=setu.inventory.fsn.xyz.analysis.report&field=stock_file_data&id=%s&filename=%s' % (
                self.id, file_name),
            'target': 'new',
        }
    ...
    def download_report_in_listview(self):
        ...
        graph_view_id = self.env.ref(
            'setu_advance_inventory_reports.setu_inventory_fsn_xyz_analysis_bi_report_graph').id
        list_view_id = self.env.ref('setu_advance_inventory_reports.setu_inventory_fsn_xyz_analysis_bi_report_list').id
