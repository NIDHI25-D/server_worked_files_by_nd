...
class SetuInventoryTurnoverAnalysisReport(models.TransientModel):
    ...
    product_ids = fields.Many2many("product.product", 'inven_turnover_ana_report_product_rel',
                                   'inven_turnover_ana_report_id', 'product_id', string="Products")
    warehouse_ids = fields.Many2many("stock.warehouse", 'inven_turnover_ana_report_warehouse_rel',
                                     'inven_turnover_ana_report_id', 'warehouse_id', string="Warehouses")
    warehouses_ids = fields.Many2many("stock.warehouse", 'inventory_turnover_ana_report_warehouse_rel',
                                      'inventory_turnover_ana_report_id',
                                      'warehouses_id',
                                      string="Warehouses",
    ...
    def download_report(self):
        ...
        return_data = {
            ...
            'target': 'new',
            'url': '/web/binary/setu_air_download_document?model=setu.inventory.turnover.analysis.report&field=stock_file_data&id=%s&filename=%s' % (
                self.id, file_name),
        }
        return return_data
    ...
    def download_report_in_listview(self):
        ...
        graph_view_id = self.env.ref(
            'setu_advance_inventory_reports.setu_inventory_turnover_analysis_bi_report_graph').id
        list_view_id = self.env.ref(
            'setu_advance_inventory_reports.setu_inventory_turnover_ratio_analysis_bi_report_list').id
