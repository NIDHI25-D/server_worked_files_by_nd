...
class SetuInventoryOutOfStockReport(models.TransientModel):
    ...
    product_ids = fields.Many2many("product.product", 'inven_outofstock_report_product_rel',
                                   'inven_outofstock_report_id', 'product_id', string="Products")
    warehouse_ids = fields.Many2many("stock.warehouse", 'inven_outofstock_report_warehouse_rel',
                                     'inven_outofstock_report_id', 'warehouse_id', string="Warehouses")
    warehouses_ids = fields.Many2many("stock.warehouse", 'inventory_outofstock_report_warehouse_rel',
                                      'inventory_outofstock_report_id', 'warehouses_id',
    ...
    def download_report(self):
        ...
        return {
            ...
            'url': '/web/binary/setu_air_download_document?model=setu.inventory.outofstock.report&field=stock_file_data&id=%s&filename=%s' % (
                self.id, file_name),
            'target': 'new',
        }
    ...
    def download_report_in_listview(self):
        ...
        list_view_id = self.env.ref('setu_advance_inventory_reports.setu_inventory_outofstock_bi_report_list').id
