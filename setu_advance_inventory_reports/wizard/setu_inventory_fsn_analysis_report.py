...
class SetuInventoryFSNAnalysisReport(models.TransientModel):
    ...
    product_ids = fields.Many2many("product.product", 'fsn_anal_report_product_rel', 'fsn_anal_report_id', 'product_id',
                                   string="Products")

    warehouse_ids = fields.Many2many("stock.warehouse", 'fsn_anal_report_warehouse_rel',
                                     'fsn_anal_report_id', 'warehouse_id', string="Warehouses")

    warehouses_ids = fields.Many2many("stock.warehouse", 'fsn_analysis_report_warehouses_rel',
                                      'fsn_analysis_report_id', 'warehouses_id',
    ...
    def download_report(self):
        ...
        return {
            ...
            'url': '/web/binary/setu_air_download_document?model=setu.inventory.fsn.analysis.report&field=stock_file_data&id=%s&filename=%s' % (
                self.id, file_name),
            'target': 'new',
        }
    ...
    def download_report_in_listview(self):
        ...
        list_view_id = self.env.ref('setu_advance_inventory_reports.setu_inventory_fsn_analysis_bi_report_list').id
...
class SetuInventoryFSNAnalysisBIReport(models.TransientModel):
    ...
    _description = """Inventory FSN Analysis Report"""
