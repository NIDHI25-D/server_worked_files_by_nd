...
class SetuInventoryCountSession(models.Model):
    ...
    user_ids_count = fields.Integer(compute="_compute_user_ids_count", string="User Count")
    ...
    approver_id = fields.Many2one(related="inventory_count_id.approver_id", string="Approver")
    ...
    def action_view_session_history(self):
        return {
            'name': 'History',
            'view_mode': 'list',
            'view_id': self.sudo().env.ref('setu_inventory_count_management.setu_inventory_session_details_list_view').id,
    ...
    def resume(self):
        ...
        date_today = datetime.now()
        self.env['inventory.session.details'].create({'session_id': self.id, 'start_date': date_today})
