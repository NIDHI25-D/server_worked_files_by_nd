...
#-------------------------------------------------------------------------------
# Make a new file:
# sudo nano setu_odoo_install.sh
# Place this content in it and then make the file executable:
# sudo chmod +x setu_odoo_install.sh
...
ADMIN_EMAIL="kishan@setuconsulting.com"
##
...
sudo mkdir $OE_HOME/logs
sudo chown $OE_USER:$OE_USER $OE_HOME/logs

#--------------------------------------------------
# Install ODOO
#--------------------------------------------------
echo -e "\n==== Installing ODOO Server ===="
sudo git clone --depth 1 --branch $OE_VERSION https://www.github.com/odoo/odoo "$OE_HOME/odoo"
...
sudo su $OE_USER -c "mkdir $OE_HOME/custom_addons"
...
sudo su $OE_USER -c "mkdir $OE_HOME/filestore"
...
sudo su root -c "printf 'logfile = $OE_HOME/logs/${OE_CONFIG}.log\n' >> $OE_HOME/conf/${OE_CONFIG}.conf"
...
if [ $IS_ENTERPRISE = "True" ]; then
    ...
    sudo su root -c "printf 'addons_path=${OE_HOME}/odoo/addons,${OE_HOME}/custom_addons\n' >> $OE_HOME/conf/${OE_CONFIG}.conf"
fi
...
sudo su root -c "echo 'sudo -u $OE_USER $OE_HOME/odoo/odoo-bin --config=$OE_HOME/conf/${OE_CONFIG}.conf' >> $OE_HOME/start.sh"
...
if [ $INSTALL_NGINX = "True" ]; then
  ...
  cat <<EOF > ~/odoo_ng.conf
  server {
  listen 80;

  # set proper server name after domain set
  server_name "system.artewhite.com";
...
EOF
  ...
  sudo mv ~/odoo_ng.conf /etc/nginx/sites-available/
  sudo ln -s /etc/nginx/sites-available/odoo_ng.conf /etc/nginx/sites-enabled/odoo_ng.conf
...
echo "Configuraton file location: $OE_HOME/conf/${OE_CONFIG}.conf"
echo "Logfile location: $OE_HOME/logs/$OE_USER"
