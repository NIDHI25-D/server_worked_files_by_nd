...
# from odoo.tools import (format_date,datetime)
from datetime import datetime
from odoo.tools import convert, format_date

import logging
_logger = logging.getLogger(__name__)
_file_logger = logging.getLogger("cancel_edi_log_format")
...
class AttachXmlsWizard(models.TransientModel):
    ...
    def _default_journal(self):
        # type_inv = 'in_invoice' if self._context.get('l10n_mx_edi_invoice_type') == 'in' else 'out_invoice'
        type_inv = 'in_invoice'
        return self.env['account.move'].with_context(type=type_inv)._default_journal()
    ...
    def _get_account_domain(self):
        #change 
        return [('company_ids', 'in', self.env.company.id), ('account_type', '!=', 'liability_payable')]
    ...
    def validate_documents(self, key, xml, account_id, account_analytic_id= False):
        ...
        # change
        xml_status = inv.l10n_mx_edi_cfdi_sat_state
        inv_vat_receiver = (
        ...
        #change
        lock_date = self.env.user.company_id._get_user_fiscal_lock_date(self.journal_id)
        date = xml.get('Fecha', '').split('T')[0]
        date_object = datetime.strptime(date, '%Y-%m-%d').date()
        if date_object <= lock_date:
            # change
        ...
        # change
        if version == '4.0' and self.env['ir.module.module'].search([('name', '=', 'l10n_mx_edi')]).state == 'installed':
            receiver = partner_obj.search(['&', ('vat', '=', xml.Receptor.attrib.get('Rfc')),  '|', ('company_id', '=', False),('company_id', '=', self.env.user.company_id.id)], limit=1)
            # if xml.attrib.get('Exportacion') != receiver.l10n_mx_edi_external_trade_type:
            #     errors.append((True, {'tradenotmatch': True}))
            if xml.Receptor.attrib.get('RegimenFiscalReceptor') != receiver.l10n_mx_edi_fiscal_regime:
                errors.append((True, {'regimenotmatch': True}))
            s = []
            [s.append({'dic': i, 'index': errors.index(i)}) if 'version' in i[1].keys() else False for i in errors]
            if s:
                errors[s[0].get('index')] = (False, {'version': True})
        elif version == '4.0':
            errors.append((True, {'notinstalled': True}))
        msg = {}
        for error in errors:
            if error[0]:
                msg.update(error[1])
        ...
        if not inv_id:
            invoice_status = self.create_invoice(
                ...
                xml_taxes.get('taxes_ids', {}), account_id, analytic_account_id=account_analytic_id)
        ...
        inv.generate_xml_attachment(xml)
    ...
    def _cancel_documents(self, files):
        cancel_uuid = []
        total_fiels = len(files)
        count = 1
        for key, xml64 in files.items():
            try:
                ...
                attachment = self.env['ir.attachment'].search([('l10n_mx_edi_cfdi_uuid','=', uuid)], limit=1)
                invoice = attachment.res_model == 'account.move' and invoice_obj.browse(attachment.res_id)
                if invoice and invoice.l10n_mx_edi_cfdi_uuid == uuid:
                    _logger.info(f"Invoice {invoice.name} Found for this UUID {uuid} ")
                    _file_logger.info(f"{uuid},Found,{invoice.partner_id.name},{invoice.partner_id.vat},{invoice.number}")
                else:
                    credentials = self.env['account.edi.format']._l10n_mx_edi_get_finkok_credentials_company(self.env.user.company_id)
                    result = self.env['account.edi.format']._l10n_mx_edi_finkok_cancel_service(uuid,self.env.user.company_id,credentials)
                    _logger.info(f"Processing {count} out of {total_fiels} file {key} status {list(result.values())[0]}")
                    cancel_uuid.append({uuid:list(result.values())[0]})
                    _file_logger.info(
                        f"{uuid},{list(result.values())[0]},{xml.Receptor.get('Nombre')},{xml.Receptor.get('Rfc')},{xml.get('Serie')}{xml.get('Folio')}")
            except Exception as e:
                _logger.error(f"Error while processing file {key} error {e}")
            finally:
                count +=1
        return {'cancelDocs': cancel_uuid, 'invoices': [], 'wrongfiles':[]}
    ...
    def check_xml(self, files, account_id=False):
        ...
        :rtype: dict
        """
        if self._context.get('cancel_documents'):
            return self._cancel_documents(files)
        ...
        account_analytic_id = self._context.get('account_analytic_id', False)
        for key, xml64 in files.items():
            try:
                if isinstance(xml64, bytes):
                    xml64 = xml64.decode()
            ...
            validated_documents = self.validate_documents(key, xml, account_id, account_analytic_id= account_analytic_id)
    ...
    def get_default_analytic(self, product, supplier):
        try:
            analytic_default = self.env['account.analytic.distribution.model']
        except BaseException:
            return False
        default_analytic = analytic_default._get_distribution({'product_id': product.id, 'partner_id': supplier.id,
                                                                'company_id': self.env.user.company_id.id})
        return default_analytic

    def create_invoice(
            self, xml, supplier, currency_id, taxes, account_id=False, analytic_account_id=False):
        ...
        # change
        journal = self._context.get('custom_journal_id', False)
        ...
        account_id = account_id or line_obj.with_context({
            'custom_journal_id': journal.id, 'type': 'in_invoice'})._default_account().id
        invoice_line_ids = []
        ...
        for idx, rec in enumerate(xml.Conceptos.Concepto):
            ...
            if not analytic_account_id:
                analytic_account_id = self.get_default_analytic(product_id, supplier)
            invoice_line_ids.append((0, 0, {
                'product_id': product_id.id,
                'account_id': account_id,
                'analytic_distribution': {str(analytic_account_id): 100.0} if type(analytic_account_id) == int else analytic_account_id,
                'name': name,
        ...
        invoice_id.generate_xml_attachment(xml)
        invoice_id._compute_cfdi_values()
        #commented as per request to keep it in draft.
