...
class SaleOrder(models.Model):
    ...
    def meli_deliver(self, meli=None, config=None, data=None):
        ...
        res = {}
        if getattr(config._fields, 'mercadolibre_stock_mrp_production_process',
                   False) and config.mercadolibre_stock_mrp_production_process:
            self.meli_produce(meli=meli, config=config, data=data)

        for spick in self.picking_ids._origin:
            try:
