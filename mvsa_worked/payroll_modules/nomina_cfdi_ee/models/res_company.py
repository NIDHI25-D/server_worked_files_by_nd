...
class ResCompany(models.Model):
    ...
    def calculate_contract_vacaciones(self, contract):
        ...
        # TASK: Automatically Vacation allocations (https://app.clickup.com/t/86dqzqpkq)
        # TASK: Generation of Annual Vacation Periods (https://app.clickup.com/t/86dxta45t)
        # ir.config_parameter taking wrong value. as per odoo 16 make changes here
        if self.env['ir.config_parameter'].sudo().get_param('nomina_cfdi_ee.automatic_vacation_id') :
            allocation_vals = {
                'name': 'Year Allocation',
                # 'holiday_type': 'employee', removed because field is removed form model
                'employee_id':contract.employee_id.id,
                # 'employee_ids': [contract.employee_id.id], removed because field is removed form model
                'holiday_status_id': int(self.env['ir.config_parameter'].sudo().get_param('nomina_cfdi_ee.automatic_vacation_id')) or False,
                'number_of_days': tablas_cfdi_line.vacaciones,
                'state': 'confirm',
                'date_from':contract.date_start.replace(year=int(current_year)),
                'date_to': contract.date_start.replace(year=int(current_year)) + timedelta(days=365),
            }
