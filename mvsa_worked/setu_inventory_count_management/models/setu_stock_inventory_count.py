...
class StockInvCount(models.Model):
    ...
    inventory_count_date = fields.Date(default=fields.Datetime.now, string="Inventory Count Date")
    ...
    discrepancy_ratio = fields.Float(compute="_compute_discrepancy_ratio", string="Discrepancy Ratio")
    user_mistake_ratio = fields.Float(compute="_compute_user_mistake_ratio", string="User Mistake Ration")
    ...
    def _compute_approver_id(self):
        for record in self:
            users = self.env['res.users'].search([('groups_id', 'in', self.env.ref(
