...
class SetuInventoryAgeReport(models.TransientModel):
    ...
    product_ids = fields.Many2many("product.product", 'inven_age_report_product_rel', 'inven_age_report_id',
    ...
    def get_inventory_age_report_data(self):
        ...
        query = """select * from inventory_stock_age_report('%s','%s','%s')""" % (
    ...
    def download_report(self):
        ...
        return {
            ...
            'url': '/web/binary/setu_air_download_document?model=setu.inventory.age.report&field=stock_file_data&id=%s&filename=%s' % (
                self.id, file_name),
            'target': 'new',
        }

    def download_report_in_listview(self):
        self._cr.execute("Drop Table if exists categ_costing_method_table;")
        ...
        """)
        category_ids = self.env['product.category'].search([('property_cost_method', '=', False)])
        for category_id in category_ids:
            print(category_id.property_cost_method)
        ...
        list_view_id = self.env.ref('setu_advance_inventory_reports.setu_inventory_age_bi_report_list').id
