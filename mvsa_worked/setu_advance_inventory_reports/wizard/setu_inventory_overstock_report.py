...
class SetuInventoryOverstockReport(models.TransientModel):
    ...
    product_ids = fields.Many2many("product.product", 'inven_overstock_report_product_rel', 'inven_overstock_report_id',
                                   'product_id', string="Products")

    warehouse_ids = fields.Many2many("stock.warehouse", 'inven_overstock_report_warehouse_rel',
                                     'inven_overstock_report_id',
                                     'warehouse_id', string="Warehouses")
    warehouses_ids = fields.Many2many("stock.warehouse", 'inventory_overstock_report_warehouses_rel',
                                      'inventory_overstock_report_id',
    ...
    def download_report(self):
        ...
        return {
            ...
            'url': '/web/binary/setu_air_download_document?model=setu.inventory.overstock.report&field=stock_file_data&id=%s&filename=%s' % (
                self.id, file_name),
            'target': 'new',
        }
    ...
    def download_report_in_listview(self):
        ...
        list_view_id = self.env.ref('setu_advance_inventory_reports.setu_inventory_overstock_bi_report_list').id
    ...
    def write_data_to_worksheet(self, workbook, worksheet, data, row):
        ...
        odd_normal_right_format_with_date = self.set_format(workbook,
                                                            setu_excel_formatter.ODD_FONT_MEDIUM_NORMAL_RIGHT_WITH_DATE)
