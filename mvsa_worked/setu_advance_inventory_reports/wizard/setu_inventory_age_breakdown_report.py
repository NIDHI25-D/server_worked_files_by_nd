...
class SetuInventoryAgeBreakdownReport(models.TransientModel):
    ...
    product_ids = fields.Many2many("product.product", 'inven_age_breakdown_report_product_rel',
                                   'inven_age_breakdown_report_id', 'product_id', string="Products")
    ...
    def download_report(self):
        ...
        return {
            ...
            'url': '/web/binary/setu_air_download_document?model=setu.inventory.age.breakdown.report&field=stock_file_data&id=%s&filename=%s' % (
                self.id, file_name),
            'target': 'new',
        }
    ...
    def download_report_in_listview(self):
        ...
        tree_view_id = self.env.ref('setu_advance_inventory_reports.setu_inventory_age_breakdown_bi_report_list').id
        ...
        return {
            ...
            'res_model': 'setu.inventory.age.breakdown.bi.report',
            'view_mode': viewmode,
    ...
    def create_data(self, data):
        ...
        return self.env['setu.inventory.age.breakdown.bi.report'].create(data)
    ...
    def write_report_data_header(self, workbook, worksheet, row):
        ...
        self.set_breakdown_header(workbook, worksheet, row - 1, 10, self.get_column_header(4),
                                  even_normal_center_format)
        ...
        self.set_breakdown_header(workbook, worksheet, row - 1, 14, self.get_column_header(6),
                                  even_normal_center_format)
...
class SetuInventoryAgeBreakdownBIReport(models.TransientModel):
    _name = 'setu.inventory.age.breakdown.bi.report'
    _description = """Inventory Age BreakdownAnalysis Report"""
    ...
    total_stock_value = fields.Float("Total Stock Value")
    breakdown1_qty = fields.Float("Stock Breakdown 1")
    breakdown2_qty = fields.Float("Stock Breakdown 2")
    breakdown3_qty = fields.Float("Stock Breakdown 3")
    breakdown4_qty = fields.Float("Stock Breakdown 4")
    breakdown5_qty = fields.Float("Stock Breakdown 5")
    breakdown6_qty = fields.Float("Stock Breakdown 6")
    breakdown7_qty = fields.Float("Stock Breakdown 7")
    breckdown1_value = fields.Float("Value Breakdown 1")
    breckdown2_value = fields.Float("Value Breakdown 2")
    breckdown3_value = fields.Float("Value Breakdown 3")
    breckdown4_value = fields.Float("Value Breakdown 4")
    breckdown5_value = fields.Float("Value Breakdown 5")
    breckdown6_value = fields.Float("Value Breakdown 6")
    breckdown7_value = fields.Float("Value Breakdown 7")
    wizard_id = fields.Many2one("setu.inventory.age.breakdown.report")
