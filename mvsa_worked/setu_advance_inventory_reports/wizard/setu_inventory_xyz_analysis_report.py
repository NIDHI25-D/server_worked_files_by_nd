...
class SetuInventoryXYZAnalysisReport(models.TransientModel):
    ...
    product_ids = fields.Many2many("product.product", 'xyz_anal_report_product_rel', 'xyz_anal_report_id', 'product_id',
                                   string="Products")
    ...
    def download_report(self):
        ...
        return {
            ...
            'url': '/web/binary/setu_air_download_document?model=setu.inventory.xyz.analysis.report&field=stock_file_data&id=%s&filename=%s' % (
                self.id, file_name),
            'target': 'new',
        }
    ...
    def download_report_in_listview(self):
        ...
        list_view_id = self.env.ref('setu_advance_inventory_reports.setu_inventory_xyz_analysis_bi_report_list').id
...
class SetuInventoryXYZAnalysisBIReport(models.TransientModel):
    ...
    _description = """Inventory XYZ Analysis Report"""
