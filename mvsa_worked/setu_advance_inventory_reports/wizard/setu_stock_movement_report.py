...
class SetuStockMovementReport(models.TransientModel):
    ...
    product_ids = fields.Many2many("product.product", 'stock_move_report_product_rel',
                                   'stock_move_report_id', 'product_id', string="Products")

    warehouse_ids = fields.Many2many("stock.warehouse", 'stock_move_report_warehouse_rel',
                                     'stock_move_report_id', 'warehouse_id', string="Warehouses")
    warehouses_ids = fields.Many2many("stock.warehouse", 'stock_movement_report_warehouse_rel',
                                      'stock_movement_report_id', 'warehouses_id',
                                      string="Warehouses", compute="_compute_warehouses_ids")
    ...
    def download_report(self):
        ...
        return {
            ...
            'url': '/web/binary/setu_air_download_document?model=setu.stock.movement.report&field=stock_file_data&id=%s&filename=%s' % (
                self.id, file_name),
            'target': 'new',
        }

    def download_report_in_listview(self):
        stock_movement_data = self.get_product_stock_movements()
        for stock_movement_data_value in stock_movement_data:
            stock_movement_data_value['wizard_id'] = self.id
            self.create_data(stock_movement_data_value)

        # graph_view_id = self.env.ref('setu_advance_inventory_reports.setu_inventory_xyz_analysis_bi_report_graph').id
        tree_view_id = self.env.ref('setu_advance_inventory_reports.setu_stock_movement_bi_report_tree').id
        ...
        return {
            ...
            'res_model': 'setu.stock.movement.bi.report',
            'view_mode': viewmode,
    ...
    def create_data(self, data):
        ...
        return self.env['setu.stock.movement.bi.report'].create(data)
...
class SetuStockMovementBIReport(models.TransientModel):
    _name = "setu.stock.movement.bi.report"
    ...
    opening_stock = fields.Float('opening_stock')
    sales = fields.Float('sales_value')
    sales_return = fields.Float('sales_return_value')
    purchase = fields.Float('purchase_value')
    purchase_return = fields.Float('purchase_return_value')
    internal_in = fields.Float('internal_in')
    internal_out = fields.Float('internal_out')
    adjustment_in = fields.Float('adjustment_in')
    adjustment_out = fields.Float('adjustment_out')
    production_in = fields.Float('production_in')
    production_out = fields.Float('production_out')
    transit_in = fields.Float('transit_in')
    transit_out = fields.Float('transit_out')
    closing = fields.Float('closing_stock')
    wizard_id = fields.Many2one("setu.stock.movement.report")
