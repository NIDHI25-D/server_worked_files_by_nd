...
BEGIN
    ...
    Select
        final_data.company_id, cmp.name, final_data.product_id,
        ('['||prod.default_code||']'||' '||(tmpl.name ->>'en_US')) as product_name,
    ...
    (
        Select
            ...
            coalesce(ls.cost_price,0) as stock_value,
        ...
        (
            ...
            Left join LATERAL get_product_company_dependent() as ls ON ls.product_id= sales_data.product_id and ls.company_id = sales_data.company_id
        Left Join
        (
            Select
                po_line.product_id,
