...
class ResCompany(models.Model):
    ...
    rest_estafeta_label_api_url = fields.Char(string="Rest Label API URL", help="Put the URl for getting the Label hear.",
                                         copy=False)
    estafeta_rate_api_url_for_multile_packages = fields.Char(string="Rate API URL For A Multiple Packages",
                                                             help="Put the URl for getting the Rate hear for multiple Packages.",
                                                             copy=False)
    estafeta_label_api_key = fields.Char(string="Label API Key",copy=False)
    estafeta_label_shared_secret = fields.Char(string="Label Shared Secret",copy=False)
    estafeta_label_api_token = fields.Char(string="Label API Token",copy=False,readonly=True)
    estafeta_tracking_api_key = fields.Char(string="Tracking API Key",copy=False)
    estafeta_tracking_shared_secret = fields.Char(string="Tracking Shared Secret",copy=False)
    estafeta_tracking_api_token = fields.Char(string="Tracking API Token",copy=False,readonly=True)
    # https://app.clickup.com/t/86dwfyv1m { Issue when multiple transfers validation. }
    estafeta_sender_phone = fields.Char(string="Sender Phone")
    estafeta_sender_email = fields.Char(string="Sender Email")
            ...
            if response.status_code in [200, 201]:
                ...
                if response_data['access_token']:
                    ...
                    return {
                        'effect': {
                            'fadeout': 'slow',
                            'message': "Yeah! Estafeta Rate Token Retrieved successfully!!",
    ...
    def estafeta_label_generate_token(self):
        api_url = (
            "{0}?grant_type=client_credentials&client_id={"
            "1}&client_secret={2}&scope=execute").format(self.estafeta_token_api_url,
                                                         self.estafeta_label_api_key, self.estafeta_label_shared_secret)
        headers = {"Content-Type": "application/x-www-form-urlencoded"}
        payload = {}
        try:
            response = requests.request("POST", api_url, headers=headers, data=payload)
            if response.status_code in [200, 201]:
                response_data = response.json()
                if response_data['access_token']:
                    self.estafeta_label_api_token = response_data['access_token']
                    return {
                        'effect': {
                            'fadeout': 'slow',
                            'message': "Yeah! Estafeta Label Token Retrieved successfully!!",
                            'img_url': '/web/static/img/smile.svg',
                            'type': 'rainbow_man',
                        }
                    }
    ...
    def estafeta_tracking_generate_token(self):
        api_url = (
            "{0}?grant_type=client_credentials&client_id={"
            "1}&client_secret={2}&scope=execute").format(self.estafeta_token_api_url,
                                                         self.estafeta_tracking_api_key, self.estafeta_tracking_shared_secret)
        headers = {"Content-Type": "application/x-www-form-urlencoded"}
        payload = {}
        try:
            response = requests.request("POST", api_url, headers=headers, data=payload)
            if response.status_code in [200, 201]:
                response_data = response.json()
                if response_data['access_token']:
                    self.estafeta_tracking_api_token = response_data['access_token']
                    return {
                        'effect': {
                            'fadeout': 'slow',
                            'message': "Yeah! Estafeta Tracking Token Retrieved successfully!!",
                            'img_url': '/web/static/img/smile.svg',
                            'type': 'rainbow_man',
                        }
                    }
        ...
        for credential_id in self.search([]):
            try:
                if credential_id.use_estafeta_shipping_provider:
                    ...
                    credential_id.estafeta_label_generate_token()
                    credential_id.estafeta_tracking_generate_token()
            except Exception as e:
