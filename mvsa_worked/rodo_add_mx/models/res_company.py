...
class ResCompany(models.Model):
    ...
    synchronize = fields.Boolean(
        string='Sincronizado',
    )
    rfc_license = fields.Char(
        string='RFC licencia',
    )
    ...
    @api.model
    def auto_verifica_solicitud_descarga(self):
        for company in self.search([('l10n_mx_esignature_ids', '!=', False)]):
            
            solicitud_ws_ids = self.env['solicitud.ws'].search([('cod_estatus','=', '5000'), ('state','=', 'draft'), ('company_id','=', company.id)])
            if solicitud_ws_ids:
                for solicitud_ws in solicitud_ws_ids:
                    solicitud_ws.get_descargar()
                    

        return True
    

    def generar_solicitud(self, date_from, date_to, rfc_receptor):
        for rec in self:
            if rec.synchronize:
                for esignature in rec.l10n_mx_esignature_ids:
                    if not esignature.content:
                        raise UserError(_('Archivo .cer path is missing.'))
                    if not esignature.key:
                        raise UserError(_('Archivo .key path is missing.'))
                    
                    headers = {
                        'Authorization': rec.token.replace('(', '').replace(')', '').replace("'", '').replace(",", ''),
                    ...
                    json_data = {
                        'type': 'create',
                        'rfc': esignature.holder_vat,
                        'contrasena': esignature.password,
                        'fecha_inicio': date_from.strftime('%Y-%m-%dT%H:%M:%S'),
                        'fecha_fin': date_to.strftime('%Y-%m-%dT%H:%M:%S'),
                        'tipo_solicitud': '01' if rfc_receptor == True else '02',
                    }
                    #--- PRUEBAS    ---#
                    #response = requests.post('http://localhost:8013/api/verify', headers=headers, json=json_data)
                    #--- -------    ---#

                    #--- PRODUCCION    ---#
                    response = requests.post('https://tuodoomexico.com/api/verify', headers=headers, json=json_data)
                    #--- -------    ---#

                    if response.status_code == 200:
                        if response.json()['result']['ok'] == True:
                            value = {
                                'id_solicitud': response.json()['result']['data']['id_solicitud'],
                                'cod_estatus': response.json()['result']['data']['cod_verifica'],
                                'mensaje': response.json()['result']['data']['mensaje'],

                            }
                            print("+++++    value   +++++++++", value)
                            return value
                        if response.json()['result']['ok'] == False:
                            raise UserError(_(response.json()['result']['msg']))
                    
                    else:
                        value = {
                                'id_solicitud': False,
                                'cod_estatus': response.json()['value']['valor']['codigo'],
                                'mensaje': response.json()['value']['valor']['mensaje'],

                            }
                        return value
                
            else:
                raise UserError(_("No esta sincronizado, revise en contabilitad/ajustes/SAT Synchronization, que este marcado el check de sincronizado"))
            
           
                
                # v = {'id_solicitud': '1b99dc4d-1208-4845-b8f1-b3b483a06a1f', 'cod_estatus': 5000, 'mensaje': 'Solicitud Aceptada'}
                # return v
    ...
    def download_cfdi_invoices_api(self, start_date=False, end_Date=False):
        ...
        # Recibidos -- Supplier
        # token = self.soap_generate_token(sat_obj.certificate, sat_obj.private_key)
        ...
        if not solicitud_ws_ids:
           solicitud = self.generar_solicitud(date_from=date_from, date_to=date_to, rfc_receptor=True)
           solicitud_ws_ids = self.env['solicitud.ws'].create({'id_solicitud': solicitud['id_solicitud'],
                                                               'cod_estatus': 5000 if solicitud['id_solicitud'] != False else solicitud['cod_estatus'],
        ...
        solo_documentos_de_proveedor = self.env['ir.config_parameter'].sudo().get_param('rodo_add_mx.solo_documentos_de_proveedor')
        if not solo_documentos_de_proveedor:
           ...
           solicitud_ws_ids = self.env['solicitud.ws'].search([('fecha_inicio','=', date_from.date()), ('fecha_fin','=', date_to.date()), ('rfc_emisor','=', True), 
                                                               ('state','=', 'draft'), ('company_id', '=', self.id)], limit=1)
           if not solicitud_ws_ids:
                   solicitud = self.generar_solicitud(date_from=date_from, date_to=date_to, rfc_receptor=False)
                   solicitud_ws_ids = self.env['solicitud.ws'].create({'id_solicitud': solicitud['id_solicitud'],
                                                                  'cod_estatus': 5000 if solicitud['id_solicitud'] != False else solicitud['cod_estatus'],
                                                                  'mensaje': solicitud['mensaje'],
                                                                  'fecha_inicio': date_from,
...
#        for _ in range(10):
        ...
        if estado_solicitud == 3:
                for paquete in verificacion['paquetes']:
                    ...
                    if descarga['cod_estatus'] != '5008':
                       content.append(descarga['paquete_b64'])
                    else:
                        solicitud_ws.write({'cod_verifica': descarga['cod_estatus'], 
                                            'mensaje_ver': descarga['mensaje'],
                                            'state': 'cancel'})
