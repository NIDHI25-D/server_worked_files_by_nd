...
class StockQuant(models.Model):
    ...
    def _update_available_quantity(self, product_id, location_id, quantity=False, reserved_quantity=False, lot_id=None,
        ...
        """
            ...
            Purpose: This method will check whether the mentioned quantity in stock.quant is negative/positive.
                     If the quantity is negative, then it will raise error. This will work only if the boolean in config is enabled.
                    This method is called when negative stock is given in counted quantity of stock.quant and clicking on Apply all.
        """
        if not location_id.allow_negative_stock and location_id.usage == "internal":
            # gather method will give record of stock.quant. Method is mentioned in base
            quants = self._gather(
