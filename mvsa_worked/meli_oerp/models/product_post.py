...
class product_template_post(models.TransientModel):
    ...
    # Done with development related to the import category and price error while click on the suggested category button.
    ml_store_id = fields.Many2one('mercadolibre.store', string="Ml Store")
    #*#
    ...
    def product_template_post(self, context=None):
        ...
        # custom changes
        # store_id https://app.clickup.com/t/865cf9d88
        if self.ml_store_id:
            custom_context.update({'ml_selected_store_id': self.ml_store_id.store_id})
        #*#
        posted_products = 0
...
class product_post(models.TransientModel):
    ...
    action_pause = fields.Boolean(string="Pausar producto",help="No actualiza el producto completo, sólo pausa el producto",default=False)
    # custom changeshttps://app.clickup.com/t/865cf9d88
    # Done with development related to the import category and price error while click on the suggested category button.
    ml_store_id = fields.Many2one('mercadolibre.store', string="Ml Store")
    #*#
