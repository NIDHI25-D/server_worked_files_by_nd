...
class sale_order(models.Model):
    ...
    def _meli_status_brief(self):
        _logger.debug("Compute _meli_status_brief method start")
        for order in self:
        ...
        _logger.debug("Compute _meli_status_brief method end")
...
class mercadolibre_orders(models.Model):
    ...
    def full_alt_phone( self, buyer_json, context=None ):
        full_phone = ''
        # https://app.clickup.com/t/86drr13tu?comment=90170022978936
        # Solve the error when order is not importing.
        phone_json = []
        #*#
        if buyer_json:
    ...
    def orders_update_order_json( self, data, context=None, config=None, meli=None ):
        ...
        # to resolve the isuue of partner invoice
        #'partner_invoice_id': (partner_invoice_id and partner_invoice_id.id), to 
        #'partner_invoice_id': company.mercadolibre_invoice_address_id.id if company.mercadolibre_invoice_address_id else partner_invoice_id.id,
        ...
        if (order and order.id):
            ...
            # https://app.clickup.com/t/865ckuw81?comment=90080037522771
            # Create ML orders with the superuser.
            # order = order_obj.create( (order_fields))
            order = order_obj.with_user(SUPERUSER_ID).create((order_fields))
        ...
        if (sorder and sorder.id):
            ...
            if 'pack_order' in order_json["tags"] and order.shipping_id:
                ...
                # Create ML orders with the superuser.
                sorder = saleorder_obj.with_user(SUPERUSER_ID).create((meli_order_fields))
                #*#
                if sorder:
                    sorder.meli_fix_team( meli=meli, config=config )
    ...
    def orders_query_iterate( self, offset=0, context=None, config=None, meli=None, fetch_id_only=False, fetch_ids=[] ):
        ...
        #custom chnages https://app.clickup.com/t/86dr7k5zr?comment=90170012380177
        # Change in the meli api order and add date_from api for orders.
        # orders_query = "/orders/search?seller="+str(meli.seller_id)+"&sort=date_desc"
        dt_time = datetime.strptime(str(config.mercadolibre_filter_order_datetime), "%Y-%m-%d %H:%M:%S")
        # format the datetime object to a string in ISO 8601 format
        dt = dt_time.isoformat(timespec="milliseconds") + "-00:00"

        #https://app.clickup.com/t/86dqu4a6d
        # Done with latest changes related to the delivery is not generated in the sales order while import the mercadolibre orders.
        if not fetch_id_only:
            orders_query = "/orders/search?seller=" + str(meli.seller_id) + "&sort=date_desc"
        else:
            orders_query = f"/orders/{fetch_id_only}"
        #*#
        ...
        if fetch_id_only:
            orders_json = {'results': [orders_json]}
        #*#
        ...
        if "results" in orders_json:
            for order_json in orders_json["results"]:
                if order_json:
                    ...
                    if False and "id" in order_json and fetch_id_only:
                    #*#
    ...
    def orders_query_recent( self, meli=None, config=None, fetch_id_only=False ):
        ...
        #https://app.clickup.com/t/865cf9d88
        # Check the crons of the meli :-
        # Cron meli orders
        # Cron Meli Process Get Products
        # Cron meli process post stock
        # Get meli state
        __fetch_ids = None
        try:
