...
class product_template(models.Model):
    ...
    # custom changes https://app.clickup.com/t/865cf9d88
    def product_template_post(self):
        ...
        if ("force_meli_active" in self.env.context):
            ...
            custom_context = {"force_meli_pub": force_meli_pub, "force_meli_active": force_meli_active}
        if self._context.get('ml_selected_store_id'):
            custom_context.update({'ml_selected_store_id': self._context.get('ml_selected_store_id')})
        ...
        for product in self:
            if (product.meli_pub_as_variant):
                ...
                for variant in product.product_variant_ids:
                    # custom changes https://app.clickup.com/t/865cf9d88
                    # Reformate the code of meli_oerp and remove un-necessary code from each file.
                    # Fix the bugs from the module.
                    # _logger.info("Variant:", variant, variant.meli_pub)
                    _logger.info("Variant:"+str(variant.meli_pub))
                    #*#
        ...
        if (posted_products == 0):
            ret = warningobj.info(title='MELI WARNING',
                                  message="Se intentaron publicar 0 productos. Debe forzar las publicaciones o marcar el producto con el campo Meli Publication, debajo del titulo.",
                                  message_html="")
    ...
    def product_template_stats(self):
        _logger.debug("Compute product_template_stats method start")
        for product in self:
            _pubs = ""
            _stats = ""
            for variant in product.product_variant_ids:
        ...
        _logger.debug("Compute product_template_stats method end")
        return {}
    ...
    # custom changes https://app.clickup.com/t/865ckuw81?comment=90080038018983
    def get_title_for_category_predictor(self):
        # return self.name
        return self.meli_title or self.name #*#
    ...
    def action_category_predictor(self):
        ...
        # custom changes
        #https://app.clickup.com/t/86drr13tu?comment=90170022792717
        # Solve the error while upload the product to the meli.
        # made change to get meli_categ = [] so we can't access directlt meli_categ.ids
        meli_categ, rjson = self._get_meli_category_from_predictor()
        ml_cate = self.env['mercadolibre.category']
        if meli_categ:
            for meli_cat_id in meli_categ:
                ml_cate += meli_cat_id
        if meli_categ:
            action = {
                ...
                'context': {'default_product_tmpl_id': self.id, 'default_categories_meli': ml_cate.ids},
                'target': 'new'
            }
    ...
    def _get_meli_category_from_predictor(self):
        ...
        # to remove the Following code https://app.clickup.com/t/865cf9d88
        # vals = [{
...
class product_product(models.Model):
    ...
    def product_get_meli_update( self ):
        _logger.debug("Compute product_get_meli_update method start")
        ...
        _logger.debug("Compute product_get_meli_update method end")
    ...
    def product_post(self):
        ...
        for product in self:
            ...
            ml_store_id = self._context.get('ml_selected_store_id', False)
            res.append(product.with_context(ml_store_id=ml_store_id)._product_post())
            #*#
        return res
    ...
    def _product_post(self, meli=None, config=None):
        ...
        force_template_description = (config.mercadolibre_product_template_override_variant
                                      ...
                                      and config.mercadolibre_product_template_override_method in ['default',
                                                                                                   'description',
                                                                                                   'title_and_description']
                                      )
        if force_template_description or product_tmpl.meli_description == False or (
                product_tmpl.meli_description and len(product_tmpl.meli_description) == 0):
            parser = MyHTMLParser()
            ...
            if len(product_tmpl.meli_pub_variant_attributes):
                    ...
                    for value in att_value_ids(product):
                        if (value.attribute_id.id == line.attribute_id.id):
                            ...
                            if (not product_tmpl.meli_pub_as_variant):
                                product.meli_title = string.replace(product.meli_title,
                ...
                if ((not product_tmpl.meli_pub_as_variant) and values):
                    try:
                        ...
                        product.meli_title = string.replace(product.meli_title, product.name,
                                                            product.name + " " + values)
                        # pass; #*#
                        pass
        ...
        force_template_title = (config.mercadolibre_product_template_override_variant
                                and config.mercadolibre_product_template_override_method
                                and config.mercadolibre_product_template_override_method in ['title',
                                                                                             'title_and_description']
                                )
        ...
        if (product.meli_title and len(product.meli_title) > 60):
            return warningobj.info(title='MELI WARNING', message="La longitud del título (" + str(
                len(product.meli_title)) + ") es superior a 60 caracteres.", message_html=product.meli_title)
        ...
        if (product_tmpl.meli_brand == False or len(product_tmpl.meli_brand) == 0):
            product_tmpl.meli_brand = (
                    "product_brand_id" in product_tmpl._fields and product_tmpl.product_brand_id and product_tmpl.product_brand_id.id and product_tmpl.product_brand_id.name)
        ...
        if product_tmpl.attribute_line_ids:
            ...
            for at_line_id in product_tmpl.attribute_line_ids:
                ...
                if (len(at_line_id.value_ids) == 1):
                    ...
                    if (
                            at_line_id.attribute_id.meli_default_id_attribute.id and at_line_id.attribute_id.meli_default_id_attribute.variation_attribute == True and at_line_id.attribute_id.meli_default_id_attribute.hidden == False):
        ...
        if (product.meli_category):
            if (product.meli_category.catalog_domain_chart_active):
                ...
                rjson_charts = product.meli_category.get_search_chart(meli=meli, brand=product.meli_brand,
                                                                      gender=product.meli_gender)
        ...
        # store id https://app.clickup.com/t/865ckuw81?comment=90080038018983
        # Done the changes same as the MDH Product file and sales order created by odoo bot by default from meli.
        # if config.mercadolibre_official_store_id:
        #     body["official_store_id"] = config.mercadolibre_official_store_id
        if self._context.get('ml_store_id'):
            body["official_store_id"] = self._context.get('ml_store_id')
        #*#
        ...
        if first_image_to_publish == None:
            return warningobj.info(title='MELI WARNING',
                                   message="Debe cargar una imagen de base en el producto, si chequeo el 'Dont use first image' debe al menos poner una imagen adicional en el producto.",
                                   message_html="")
        else:
            ...
            if "status" in resim:
                if (resim["status"] == "error" or resim["status"] == "warning"):
                    ...
                    if (resim["status"] == "error"):
                        return warningobj.info(title='MELI WARNING',
                                               message="Problemas cargando la imagen principal.",
                                               message_html=error_msg, context={"rjson": resim})
                else:
                    ...
                    if (len(attributes)):
                        ...
                        for att in attributes_ml:
                            if (att["id"] in dicatts):
                                ...
                                if ((att["id"] == "GTIN" or att[
                                    "id"] == "SELLER_SKU") and product_tmpl.meli_pub_as_variant):
        ...
        if (variant_image_ids(product) or template_image_ids(product) or banner_images):
            ...
            if 'status' in multi_images_ids:
                ...
                return warningobj.info(title='MELI WARNING', message="Error publicando imagenes",
                                       message_html="Error: " + str(multi_images_ids),
                                       context={"rjson": multi_images_ids})
        ...
        if (product_tmpl.meli_pub_as_variant):
                    ...
                    if (productjson and len(productjson["variations"])):
                        ...
                        for ix in range(len(productjson["variations"])):
                            ...
                            for pvar in product_tmpl.product_variant_ids:
                                if (pvar._is_product_combination(var_info)):
                                    ...
                                    var_product.meli_available_quantity = var_product._meli_available_quantity(
                                        meli=meli, config=config)

                                    # adding variant images
                                    var_product.product_meli_upload_image(meli=meli, config=config)
                                    var_multi_images_ids = var_product.product_meli_upload_multi_images(meli=meli,
                                                                                                        config=config)
                                    ...
                                    # TODO: add SKU
                                    var_attributes = var_product._update_sku_attribute(
                                        attributes=("attributes" in var_info and var_info["attributes"]) or [],
                                        set_sku=config.mercadolibre_post_default_code,
                                        set_barcode=config.mercadolibre_post_barcode,
                                        var_info=var_info)
                            ...
                            if not var_product:
                                ...
                                for pvar in product_tmpl.product_variant_ids:
                                    verb += " ##" + str(pvar) + " >>> " + str(
                                        pvar._is_product_combination(var_info, verbose=True))
                                _logger.error(verb)
                        ...
                        _new_candidates = product_tmpl.product_variant_ids.filtered(
                            lambda pv: pv.id not in _updated_ids)
                        ...
                        if _all_variations:
                            for aix in range(len(_all_variations)):
                                ...
                                for pvar in _new_candidates:
                                    if (pvar._is_product_combination(var_info)):
                                        var_attributes = pvar._update_sku_attribute(
                                            attributes=("attributes" in var_info and var_info["attributes"]),
                                            set_sku=config.mercadolibre_post_default_code, var_info=var_info)
                        ...
                        _logger.info(varias)
                        responsevar = product.meli_id and meli.put("/items/" + product.meli_id, varias,
                                                                   {'access_token': meli.access_token})
                        rjsonv = responsevar and responsevar.json()
                        _logger.info(rjsonv)
                        if (rjsonv and "error" in rjsonv):
                            error_msg = 'MELI RESP.: <h6>Mensaje de error</h6><br/><h6>Mensaje</h6> %s<br/><h6>Status</h6> %s<br/><h6>Cause</h6> %s<br/><h7>Error completo:</h7><span>%s</span><br/>' % (
                                rjsonv["message"], rjsonv["status"], rjsonv["cause"], rjsonv["error"])
                            _logger.error(error_msg)
                            if (rjsonv["error"] == "forbidden"):
                                url_login_meli = meli.auth_url()
                                return warningobj.info(title='MELI WARNING',
                                                       message="Debe iniciar sesión en MELI con el usuario correcto.",
                                                       message_html="<br><br>" + error_msg,
                                                       context={"rjson": rjsonv})
                            else:
                                return warningobj.info(title='MELI WARNING',
                                                       message="Completar todos los campos y revise el mensaje siguiente.",
                                                       message_html="<br><br>" + error_msg,
                                                       context={"rjson": rjsonv})
                        ...
                        resdes = product.meli_id and meli.put("/items/" + product.meli_id + "/description",
                                                              bodydescription, {'access_token': meli.access_token})
                        # _logger.debug(resdes.json())
                        del body['price']
                        del body['available_quantity']
                        del body["pictures"]
                        _logger.info("update post 1:" + str(body))
                        resbody = product.meli_id and meli.put("/items/" + product.meli_id, body,
                                                               {'access_token': meli.access_token})
            ...
            if (productjson and len(productjson["variations"]) == 1):
                ...
                for ix in range(len(productjson["variations"])):
                    ...
                    var_attributes = product._update_sku_attribute(
                        attributes=("attributes" in var_info and var_info["attributes"]),
                        set_sku=config.mercadolibre_post_default_code,
                        set_barcode=config.mercadolibre_post_barcode,
                        var_info=var_info)
                ...
                # _logger.debug(responsevar.json())
                resdes = product.meli_id and meli.put("/items/" + product.meli_id + "/description", bodydescription,
                                                      {'access_token': meli.access_token})
                # _logger.debug(resdes.json())
                del body['price']
                del body['available_quantity']
        ...
        try:
            if product.meli_id:
                ...
                resdescription = meli.put("/items/" + str(product.meli_id) + "/description", bodydescription,
                                          {'access_token': meli.access_token})
        ...
        if "error" in rjson:
            ...
            error_msg = '<h6>Mensaje de error de MercadoLibre</h6><br/><h2>Mensaje: %s</h2><br/><h6>Status</h6> %s<br/><h6>Cause</h6> %s<br/><h6>Error completo:</h6><br/><span>%s</span><br/>' % (
                rjson["message"], rjson["status"], rjson["cause"], rjson["error"])
            _logger.error(error_msg)
            if (rjson["cause"] and rjson["cause"][0] and "message" in rjson["cause"][0]):
                error_msg += '<h3>' + str(rjson["cause"][0]["message"]) + '</h3>'
            # expired token
            if "message" in rjson and (
                    rjson["error"] == "forbidden" or rjson["message"] == 'invalid_token' or rjson[
                "message"] == "expired_token"):
                url_login_meli = meli.auth_url()
                return warningobj.info(title='MELI WARNING',
                                       message="Debe iniciar sesión en MELI:  " + str(rjson["message"]),
                                       message_html="<br><br>" + error_msg, context={"rjson": rjson})
            else:
                # Any other errors
                return warningobj.info(title='MELI WARNING',
                                       message="Recuerde completar todos los campos y revise el mensaje siguiente.",
                                       message_html="<br><br>" + error_msg, context={"rjson": rjson})
        ...
        posting_fields = {'posting_date': str(datetime.now()), 'meli_id': product.meli_id, 'product_id': product.id,
