...
class mercadolibre_shipment(models.Model):
    ...
    def _update_sale_order_shipping_info( self, order, meli=None, config=None ):
        ...
        for shipment in self:
            ...
            for st_pick in stock_pickings:
                ...
                """
                Field is cleared by odoobot [ https://app.clickup.com/t/86duwpqdv ]
                Branch : field_is_cleared_by_odoobot_mvsa
    ...
    # Overriding methods to add a custom changes
    # Return shipment object based on mercadolibre.orders "order"
        ...
        if (response):
            ...
            if "error" in ship_json:
                ...
                if meli.access_token == "PASIVA":
                    ...
                    rescosts = meli.get("/shipments/" + str(ship_id) + str('/costs'),
                                        {'access_token': meli.access_token})
                if rescosts:
                    rcosts = rcosts or rescosts.json()
                    ship_json['costs'] = rcosts
                    recdiscounts = 'receiver' in rcosts and 'discounts' in rcosts['receiver'] and rcosts['receiver'][
                        'discounts']
                    if recdiscounts:
                ...
                ship_fields = {
                    "name": "MSO [" + str(ship_id) + "] " + str("") + str(ship_json["status"]) + "/" + str(
                        ship_json["substatus"]) + str(""),
                    'company_id': company.id,
                    'seller_id': seller_id,
                    "order": order.id,
                    ...
                    "shipping_cost": ("cost" in ship_json["shipping_option"] and ship_json["shipping_option"][
                        "cost"]) or 0.0,
                    "shipping_list_cost": ("list_cost" in ship_json["shipping_option"] and ship_json["shipping_option"][
                        "list_cost"]) or 0.0,
                ...
                if "receiver_address" in ship_json and ship_json["receiver_address"]:
                    ship_fields.update({
                        ...
                        "receiver_zip_code": (("zip_code" in ship_json["receiver_address"]) and
                                              ship_json["receiver_address"]["zip_code"]) or False
                    })
                    receiver_phone = (
                            "receiver_phone" in ship_json["receiver_address"] and ship_json["receiver_address"][
                        "receiver_phone"] and not "XXXX" in ship_json["receiver_address"]["receiver_phone"] and
                            ship_json["receiver_address"]["receiver_phone"])
                    if receiver_phone:
                    ...
                    try:
                        ...
                        if (1 == 1 and shipment.pdf_filename):
                            ...
                            if (shipment.pdfimage_filename == False):
                                ...
                                for image in images:
                                    image_filename = "/tmp/%s-page%d.jpg" % (
                                        "Shipment_" + shipment.shipping_id, images.index(image))
                        ...
                        _logger.info(
                            "Error converting pdf to jpg: try installing pdf2image and poppler-utils, like this:")
                ...
                if (full_orders and ship_fields["pack_order"]):
                    ...
                    if (config and config.mercadolibre_pricelist):
                        ...
                        error = {
                            "error": "orders_update_order_json (shipment) > no pricelist defined. Check config pricelist config: " + str(
                                config and config.name) + " pricelist: " + str(
                                config and config.mercadolibre_pricelist)}
                        _logger.error(error)
                    ...
                    # custom changes ([FIX] invoice partner address)
                    if not company.mercadolibre_invoice_address_id:
                        partner_invoice_id = respartner_obj.search(
                            [('meli_order_id', '=', partner_invoice_meli_order_id)], limit=1) or partner_id
                    else:
                        partner_invoice_id = company.mercadolibre_invoice_address_id
                    #*#
                    ...
                    if "receiver_address" in ship_json:
                        if config.mercadolibre_cron_get_orders_shipment_client:
                            partner_shipping_id = self.partner_delivery_id(partner_id=partner_id,
                                                                           Receiver=ship_json["receiver_address"],
                                                                           config=config)

                    mercadolibre_contact_partner_id = (
                            "mercadolibre_contact_partner" in config._fields and config.mercadolibre_contact_partner)
                    if (mercadolibre_contact_partner_id):
                        mercadolibre_contact_partner_id.meli_update_forbidden = True

                    mercadolibre_invoice_partner_id = (
                            "mercadolibre_invoice_partner" in config._fields and config.mercadolibre_invoice_partner)
                    if (mercadolibre_invoice_partner_id):
                        mercadolibre_invoice_partner_id.meli_update_forbidden = True

                    mercadolibre_shipping_partner_id = (
                            "mercadolibre_shipping_partner" in config._fields and config.mercadolibre_shipping_partner)
                    ...
                    if (partner_id.id):
                        oname = "pack_id" in all_orders[0] and all_orders[0]["pack_id"] and str(
                            "ML %s" % (str(all_orders[0]["pack_id"])))
                        oname = oname or str("ML %s" % (str(all_orders[0]["order_id"])))
                        sorder_pack = self.env["sale.order"].search(
                            ['|', ('meli_order_id', '=', packed_order_ids), ('name', 'like', str(oname))],
                        ...
                        meli_order_fields = self.env['mercadolibre.orders'].prepare_sale_order_vals(
                            order_json=order_json, meli=meli, config=config, sale_order=sorder_pack, shipment=shipment)
                        ...
                        meli_order_fields.update({
                            ...
                            'meli_date_created': ml_datetime(all_orders[0]["date_created"]) or all_orders[0][
                                "date_created"],
                            'meli_date_closed': ml_datetime(all_orders[0]["date_closed"]) or all_orders[0][
                                "date_created"],
                        })
                        # TODO: agregar un campo para diferencia cada delivery res partner al shipment y orden asociado, crear un binding usando values diferentes... y listo
                        ...
                        if partner_shipping_id:
                            sorder = sorder_pack
                            shipping_partner_already_set = (
                                    sorder and sorder.partner_shipping_id and sorder.partner_shipping_id.id == partner_shipping_id.id)
                        ...
                        if (len(sorder_pack)):
                            ...
                            is_locked = (sorder_pack and sorder_pack.state in ["done"]) or (
                                    "locked" in sorder_pack._fields and sorder_pack.locked)
                            ...
                            sorder_pack = self.env["sale.order"].with_user(SUPERUSER_ID).create(meli_order_fields)
                            # *#*
                            if sorder_pack:
                                sorder_pack.meli_fix_team(meli=meli, config=config)
                                order.message_post(body=str("Sale order created (pack)!"),
                        ...
                        if (sorder_pack.id):
                            ...
                            for mOrder in all_orders:
                                ...
                                product_related_obj = mOrder.order_items and (
                                        mOrder.order_items[0].product_id or mOrder.order_items[
                                    0].posting_id.product_id)
                                if not (product_related_obj):
                                    # error = { 'error': 'No product related to meli_id '+str(Item['item']['id']), 'item': str(Item['item']) }
                                    _logger.error("Error adding order line: product not found in database: " + str(
                                        mOrder.order_items and mOrder.order_items[0]["order_item_title"]))
                                ...
                                saleorderline_item_fields = {
                                    ...
                                    'name': product_related_obj.display_name or mOrder.order_items[0][
                                        "order_item_title"],
                                }
                                if (mOrder.fee_amount):
                                    sorder_pack.meli_fee_amount = sorder_pack.meli_fee_amount + mOrder.fee_amount

                                saleorderline_item_fields.update(
                                    order._set_product_unit_price(product_related_obj, mOrder.order_items[0],
                                                                  config=config))

                                saleorderline_item_ids = saleorderline_obj.search(
                                    [('meli_order_item_id', '=', saleorderline_item_fields['meli_order_item_id']),
                                     ('meli_order_item_variation_id', '=',
                                      saleorderline_item_fields['meli_order_item_variation_id']),
                                     ('order_id', '=', shipment.sale_order.id)])

                                if not saleorderline_item_ids:
                                    if sorder_pack.amount_total < (
                                            sorder_pack.meli_paid_amount - sorder_pack.meli_coupon_amount):
                                        saleorderline_item_ids = saleorderline_obj.create((saleorderline_item_fields))
                                else:
