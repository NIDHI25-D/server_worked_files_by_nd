...
Select
    ...
    sum(case when source.usage = 'internal'
        then (sml.quantity * sol.price_unit) /CASE COALESCE(so.currency_rate, 0::numeric) WHEN 0 THEN 1.0 ELSE so.currency_rate END
    else (-1 *sml.quantity * sol.price_unit) /CASE COALESCE(so.currency_rate, 0::numeric) WHEN 0 THEN 1.0 ELSE so.currency_rate END
    ...
    SUM(
        CASE
            WHEN FALSE THEN
                ...
                CASE
                    COALESCE(so.currency_rate, 0::NUMERIC)
                    WHEN 0 THEN 1.0
...
where
     1 = case when tr_start_date is null
     ...
     else case when so.date_order >= tr_start_date then 1 else 0 end
     end
