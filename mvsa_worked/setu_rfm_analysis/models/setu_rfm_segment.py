...
class SetuRFMSegment(models.Model):
    ...
    def _compute_res_partner_ids(self):
        ...
        use : to find the ids of partner for each segment
        """
        for rec in self:
            if rec.is_template:
                query = f"""
                    SELECT * FROM 
                        res_partner 
                        where 
                            jsonb_pretty(rfm_segment_id ->'{self.env.company.id}')::integer = {rec.id} 
                            and (select jsonb_object_keys(rfm_segment_id::jsonb)::integer) = {self.env.company.id};
                """
            else:
                query = f"""
                select
            ...
            segments_partners = [x[0] for x in partners if partners]
            if segments_partners and len(segments_partners) == 1:
                if isinstance(segments_partners[0], list):
                    partner_list = [item for sublist in segments_partners for item in sublist]
                else:
                    partner_list = segments_partners
                rec.partner_ids = self.env['res.partner'].sudo().browse(partner_list)
            elif segments_partners and len(segments_partners) > 1:
                if isinstance(segments_partners, list):
                    rec.partner_ids = self.env['res.partner'].sudo().browse(segments_partners)
                elif isinstance(segments_partners[0], list):
                    partner_list = [item for sublist in segments_partners for item in sublist]
                    rec.partner_ids = self.env['res.partner'].sudo().browse(partner_list)
            else:
    ...
    def transform_to_dict(self, list_of_dict):
        """
        added by: Siddharth Vasani | On : OCt-10-2024 | Task: [903] Migration 18 : RFM Analysis - Marketing Strategy
        use : transform a list of dictionaries to access directly via its segment_id
        """
        to_return = {}
    ...
    def _compute_rfm_common(self):
        """
        added by: Siddharth Vasani | On : OCt-10-2024 | Task: [903] Migration 18 : RFM Analysis - Marketing Strategy
        use : calculates essential metrics related to customer segments, including total customers, orders, revenue, and
        their respective ratios, either for an entire company or for specific sales teams
    ...
    def write(self, vals):
        """
        added by: Siddharth Vasani | On : OCt-10-2024 | Task: [903] Migration 18 : RFM Analysis - Marketing Strategy
        use : to update fields of segment records, including handling template segments
        """
    ...
    def _compute_display_name(self):
        """
        added by: Siddharth Vasani | On : OCt-10-2024 | Task: [903] Migration 18 : RFM Analysis - Marketing Strategy
        use : to get the value of display name
        """
        for record in self:
            name = record.name if record.is_template else record.crm_team_id.name + ' -> ' + record.name if record.crm_team_id else record.name
            record.display_name = name

    def copy(self, default=None):
        """
        added by: Siddharth Vasani | On : OCt-10-2024 | Task: [903] Migration 18 : RFM Analysis - Marketing Strategy
        use : gives warning for the duplication of segment
        """
        raise RedirectWarning(_("Segments can not be Duplicated, please create a new one."))

    def _compute_use_dynamic_rules(self):
        """
        added by: Siddharth Vasani | On : OCt-10-2024 | Task: [903] Migration 18 : RFM Analysis - Marketing Strategy
        use : let user use dynamic rules
        """
    ...
    def seq_constrain(self):
        """
        added by: Siddharth Vasani | On : OCt-10-2024 | Task: [903] Migration 18 : RFM Analysis - Marketing Strategy
        use : checks that the seq (sequence) value is valid, ensuring it is greater than zero and that there are no
        duplicates for segments marked as templates
        """
        for rec in self:
            if rec.seq <= 0:
    ...
    def create(self, vals):
        """
        added by: Siddharth Vasani | On : OCt-10-2024 | Task: [903] Migration 18 : RFM Analysis - Marketing Strategy
        use : to create related configurations and records when a new segment is created
        """
        ...
        if res.is_template:
            ...
            for team in self.env['crm.team'].sudo().search(['|', ('active', '=', False), ('active', '=', True)]):
    ...
    def _calculate_leads(self):
        """
        added by: Siddharth Vasani | On : OCt-10-2024 | Task: [903] Migration 18 : RFM Analysis - Marketing Strategy
        use : calculates and updates the count of open leads for a given segment, providing insights into the current
        opportunities related to that segment
        """
    ...
    def _calculate_rfq(self):
        """
        added by: Siddharth Vasani | On : OCt-10-2024 | Task: [903] Migration 18 : RFM Analysis - Marketing Strategy
        use : it is used to determine how many open RFQs exist for the partners associated with each segment
        """
        for segment in self:
            if segment.is_template:
                ids = segment.partner_ids and segment.partner_ids.ids or []
    ...
    def _compute_customers(self):
        """
        added by: Siddharth Vasani | On : OCt-10-2024 | Task: [903] Migration 18 : RFM Analysis - Marketing Strategy
        use : to calculate and update the total of customers, orders and revenues
        """
        for segment in self:
            segment.update({
    ...
    def _calculate_ratio(self):
        """
        added by: Siddharth Vasani | On : OCt-10-2024 | Task: [903] Migration 18 : RFM Analysis - Marketing Strategy
        use : to calculate ration base on total of customers, orders and revenues
        """
    ...
    def open_mailing(self):
        """
        added by: Siddharth Vasani | On : OCt-10-2024 | Task: [903] Migration 18 : RFM Analysis - Marketing Strategy
        use : to open a window displaying mailing records and manage email
        """
        ...
        report_display_views = [(kanban_view_id, 'kanban'), (tree_view_id, 'list'), (form_view_id, 'form')]
    ...
    def open_customer(self):
        """
        added by: Siddharth Vasani | On : OCt-10-2024 | Task: [903] Migration 18 : RFM Analysis - Marketing Strategy
        use : to open a window displaying customer records and manage customer
        """
        ...
        report_display_views = [(kanban_view_id, 'kanban'), (form_view_id, 'form'), (tree_view_id, 'list')]
        if self.is_template:
    ...
    def open_orders(self):
        """
        added by: Siddharth Vasani | On : OCt-10-2024 | Task: [903] Migration 18 : RFM Analysis - Marketing Strategy
        use : view and manage sales orders related to either a template or a specific sales team
        """
    ...
    def open_leads(self):
        """
        added by: Siddharth Vasani | On : OCt-10-2024 | Task: [903] Migration 18 : RFM Analysis - Marketing Strategy
        use : to view leads linked to a segment
        """
    ...
    def open_rfqs(self):
        """
        added by: Siddharth Vasani | On : OCt-10-2024 | Task: [903] Migration 18 : RFM Analysis - Marketing Strategy
        use : to view and manage RFQs related to a specific customer segment
        """
        form_view_id = self.env.ref('sale.view_order_form').id
    ...
    def create_mailing(self):
        """
        added by: Siddharth Vasani | On : OCt-10-2024 | Task: [903] Migration 18 : RFM Analysis - Marketing Strategy
        use : the creation of a mailing campaign directed at customers associated with a particular segment
        """
    ...
    def all_open_leads(self):
        """
        added by: Siddharth Vasani | On : OCt-10-2024 | Task: [903] Migration 18 : RFM Analysis - Marketing Strategy
        use :to access and view all open leads
        """
        kanban_view_id = self.env.ref('crm.crm_case_kanban_view_leads').id
        form_view_id = self.env.ref('crm.crm_lead_view_form').id
        tree_view_id = self.env.ref('setu_rfm_analysis.setu_rfm_analysis_crm_case_tree_view_oppor').id
        report_display_views = [(tree_view_id, 'list'), (kanban_view_id, 'kanban'), (form_view_id, 'form')]
        won_stage = self.env['crm.stage'].search([('is_won', '=', True)])
        return {
            'name': 'Open Leads',
            'type': 'ir.actions.act_window',
            ...
            'context': {'search_default_rfm_segment_id': 1, 'from_rfm_segment': True,
                        'action_id': 'setu_rfm_analysis.action_all_open_leads'},
            'views': report_display_views,
    ...
    def all_sale_orders(self):
        """
        added by: Siddharth Vasani | On : OCt-10-2024 | Task: [903] Migration 18 : RFM Analysis - Marketing Strategy
        use :to access and view all sale orders
        """
        form_view_id = self.env.ref('sale.view_order_form').id
        tree_view_id = self.env.ref('sale.view_order_tree').id
        report_display_views = [(tree_view_id, 'list'), (form_view_id, 'form')]
        return {
            ...
            'res_model': 'sale.order',
            'context': {'search_default_rfm_segment_id': 1, 'from_rfm_segment': True,
                        'action_id': 'setu_rfm_analysis.action_open_all_sale_orders'},
            'views': report_display_views,
    ...
    def all_mailings(self):
        """
        added by: Siddharth Vasani | On : OCt-10-2024 | Task: [903] Migration 18 : RFM Analysis - Marketing Strategy
        use :to access and view all mailings
        """
        kanban_view_id = self.env.ref('mass_mailing.view_mail_mass_mailing_kanban').id
        form_view_id = self.env.ref('mass_mailing.view_mail_mass_mailing_form').id
        tree_view_id = self.env.ref('mass_mailing.view_mail_mass_mailing_tree').id
        ...
        return {
            ...
            'context': {'search_default_rfm_segment_id': 1,
                        'action_id': 'setu_rfm_analysis.action_open_all_mailings'},
            'views': report_display_views,
            'domain': [('rfm_segment_id', '!=', False)]
        }
    ...
    def update_segment_and_rules(self):
        """
        added by: Siddharth Vasani | On : OCt-10-2024 | Task: [903] Migration 18 : RFM Analysis - Marketing Strategy
        use : to update segment and rules
        """
    ...
    def update_customer_segment(self):
        """
        added by: Siddharth Vasani | On : OCt-10-2024 | Task: [903] Migration 18 : RFM Analysis - Marketing Strategy
        use : to update customer segment
        """
