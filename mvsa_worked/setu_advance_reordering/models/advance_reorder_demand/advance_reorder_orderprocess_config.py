...
class AdvanceReorderOrderProcessConfig(models.Model):
    ...
    def onchange_vendor_lead_days(self):
        """
            ...
            Purpose: to get order arrival date on basis of vendor lead days.
                   : also here add changes as per the 16 customisation which is taken from the version 14
        """
        ...
        if self.reorder_process_template_id:
            self.order_date = self.reorder_process_template_id.reorder_date.date()
            days = self.vendor_lead_days - 1 if self.vendor_lead_days > 0.0 else self.vendor_lead_days
            self.order_arrival_date = (self.order_date +
                                       relativedelta.relativedelta(days=days)).strftime("%Y-%m-%d")
    ...
    def onchange_order_arrival_date(self):
        """
            ...
            Date: 07/01/25
            Task: Migration to v18 from v16
            Purpose : Get advance stock start and end date on basis pf Order arrival date.
                    : also here add changes as per the 16 customisation which is taken from the version 14
        """
        if self.reorder_process_id:
        ...
        if self.reorder_process_template_id:
            self.advance_stock_start_date = (self.order_arrival_date +
                                             relativedelta.relativedelta(days=1)).strftime("%Y-%m-%d")
            days = self.reorder_process_template_id.buffer_security_days - 1 \
                if self.reorder_process_template_id.buffer_security_days > 0.0 \
                else self.reorder_process_template_id.buffer_security_days
            self.advance_stock_end_date = (self.advance_stock_start_date +
                                           relativedelta.relativedelta(days=days)).strftime("%Y-%m-%d")
