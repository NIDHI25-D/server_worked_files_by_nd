...
--    opening_stock, sales, purchase, sales_return, purchase_return, adjustment_in, adjustment_out, production_in, production_out, closing)
    ...
    with row_datas as (
            Select
                row_number() over(partition by T1.product_id, T1.company_id order by T1.inventory_date::date, T1.product_id, T1.company_id)
                             as row_id,
            ...
            From  (
                ...
                from stock_move move
                    ...
                    select T.company_id, T.company_name, T.product_id, T.product_name, T.product_category_id, T.category_name,
                           old_start_date as inventory_date, sum(T.opening_stock) as opening_stock,0,0,0,0,0,0,0,0
                ...
                -- Set default 0 value for each transaction for each products for each warehouse
                    Select T2.company_id, T2.company_name, T2.product_id, T2.product_name, T2.product_category_id, T2.category_name,
                           generate_series(start_date, end_date, '1 day'::interval)::date as inventory_date, 0 as opening_stock, 0,0,0,0,0,0,0,0

                    From (
                           Select cmp.id as company_id, cmp.name as company_name, prod.id as product_id, prod.default_code as product_name,
                               cat.id as product_category_id, cat.complete_name as category_name
                           From
...
--    SELECT * FROM summary_cte order by 2,4,8;
    ...
    Insert into setu_inventory_ledger_bi_report(company_id, product_id, product_category_id, inventory_date,
        opening_stock, sales, purchase, sales_return, purchase_return, adjustment_in, adjustment_out, production_in,
        production_out, closing, wizard_id, total_in, total_out)
    select
        d.company_id, d.product_id, d.product_category_id, d.inventory_date,
        d.opening_stock, d.sales, d.purchase, d.sales_return, d.purchase_return, d.adjustment_in, d.adjustment_out, d.production_in,
        d.production_out, d.closing, wizard_id, d.purchase + d.sales_return + d.adjustment_in + d.production_in as total_in,
    ...
    select
        ...
        line_one.opening_stock, line_one.sales, line_one.purchase, line_one.sales_return, line_one.purchase_return,
        line_one.adjustment_in, line_one.adjustment_out, line_one.production_in, line_one.production_out, line_one.closing
	from
    ...
    select
	    r2.company_id, r2.product_id, r2.product_category_id, r2.inventory_date::date, r1.closing as opening_stock,
	    r2.sales, r2.purchase, r2.sales_return, r2.purchase_return, r2.adjustment_in, r2.adjustment_out, r2.production_in,
	    r2.production_out, r2.closing
