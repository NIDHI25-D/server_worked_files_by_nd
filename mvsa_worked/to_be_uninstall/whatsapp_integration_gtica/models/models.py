...
class ConvertHtmlText(object):
    ...
    def convert_html_to_text(result_txt):
        """
            ...
            Purpose: This method is used to convert HTML to text for that byte->string is needed
        """
        # capt = b'%s' % (result_txt)
        capt = b'%s' % bytes(result_txt, 'utf-8')
...
class PurchaseOrder(models.Model):
    ...
    def send_whatsapp_step(self):
        """
            ...
            Date: 07/02/23
            Task: Migration from v14 to v16
            Purpose: This method is used for button : Send Whatsapp
        """
...
class StockPickig(models.Model):
    ...
    def _action_whatsapp_confirmed(self, message=None):
        ...
        subtype_id = self.env['ir.model.data']._xmlid_to_res_id('mail_mt_note')
        self.with_context(ctx).message_post(attachment_ids=[], body=message, message_type='notification',
                                            partner_ids=[], subtype_xmlid=None,
                                            subtype_id=subtype_id)
...
class SaleOrder(models.Model):
    ...
    def items_products(self):
        """
            ...
            Date: 07/02/23
            Task: Migration from v14 to v16
            Purpose: This method is used to collect and prepare data from the template of template_products_sale
        """
        record = self.with_context(proforma=True)
    ...
    def url_link_sale(self):
        """
            ...
            Date: 08/02/23
            Task: Migration from v14 to v16
            Purpose: This method will create an url which contains orders,access_token,port etc
            Return : URL
        """
        dominio = http.request.env['ir.config_parameter'].sudo().get_param('web.base.url')
        url_preview = self.get_portal_url()
...
class Lead(models.Model):
    ...
    def _action_whatsapp_confirmed(self, message=None):
        ...
        subtype_id = self.env['ir.model.data']._xmlid_to_res_id('mail_mt_note')
        self.with_context(ctx).message_post(attachment_ids=[], body=message,
                                            message_type='notification', partner_ids=[], subtype_xmlid=None,
                                            subtype_id=subtype_id)
...
class AccountInvoice(models.Model):
    ...
    def items_products(self):
        """
            ...
            Date: 07/02/23
            Task: Migration from v14 to v16
            Purpose: This method is used to create a template and data of invoice
        """
    ...
    def _action_whatsapp_confirmed(self, message=None):
        """
            ...
            Date: 09/02/23
            Task: Migration from v14 to v16
            Purpose: This method is used to send the message through whatsapp when clicked on Send of account.move(payment)
        """
        ...
        #                                     message_type='notification', partner_ids=[], subtype_xmlid=None,
        #                                     subtype_id=subtype_id)
        self.with_context(ctx).message_post(attachment_ids=[], body=message, message_type='notification',canned_response_ids=[],
                                            partner_ids=[],
                                            subtype_xmlid=None,
                                            subtype_id=subtype_id)
...
class account_payment(models.Model):
    ...
    def send_whatsapp_step(self):
        """
            ...
            Date: 09/02/23
            Task: Migration from v14 to v16
            Purpose: This method is called for button :  Send Whatsapp, it gets the data and template of payment is loaded
        """
        record = self.with_context(proforma=True)
