...
class whatsapp_report_product_pricelist(models.AbstractModel):
    ...
    def get_report_values(self, price_list, qty, product_categories_ids):
        """
            ...
            Date: 10/5/23
            Task: Migration v14 to v16
            Purpose: This method is used to search product of particular categories
        """
    ...
    def _get_categories(self, pricelist, products, quantities):
        """
            ...
            Purpose: This method is used to get categories if Product Categories in wizard is enable
                    It will also calculate the quantity for each product.
        """
        categ_data = []
    ...
    def _get_price(self, pricelist, product, qty):
        """
            ...
            Date: 10/5/23
            Task: Migration from v14 to v16
            Purpose: This method is used to get price as per the product and calculate as
                    per the quantity and at the end it give round figure of the product
        """
        sale_price_digits = self.env['decimal.precision'].precision_get('Product Price')
        price = pricelist._get_product_price(product, qty, False)
        if not price:
