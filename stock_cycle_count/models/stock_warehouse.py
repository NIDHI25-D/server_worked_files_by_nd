...
class StockWarehouse(models.Model):
    ...
    def _cycle_count_rules_to_compute(self):
        self.ensure_one()
        rules = self.env['stock.cycle.count.rule'].search([('rule_type', '!=', 'zero'), ('warehouse_ids', 'in', self.ids)])
        return rules
    ...
    def action_compute_cycle_count_rules(self):
        ...
        for rec in self:
            ...
            if proposed_cycle_counts:
                locations = list(set([d['location'] for d in proposed_cycle_counts]))
                for loc in locations:
                    proposed_for_loc = list(filter(lambda x: x['location'] == loc, proposed_cycle_counts))
                    ...
                    if not existing_cycle_counts and delta.days < rec.cycle_count_planning_horizon:
