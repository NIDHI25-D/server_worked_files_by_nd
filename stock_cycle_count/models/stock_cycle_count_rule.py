...
class StockCycleCountRule(models.Model):
    ...
    def _check_zero_rule(self):
        for rec in self:
            if rec.rule_type == 'zero' and len(rec.warehouse_ids) > 1:
                raise ValidationError(_('Zero confirmation rules can only have one warehouse assigned.'))
            if rec.rule_type == 'zero':
                zero_rule = self.search([('rule_type', '=', 'zero'), ('warehouse_ids', '=', rec.warehouse_ids.id)])
                if len(zero_rule) > 1:
                    raise ValidationError(_('You can only have one zero confirmation rule per warehouse.'))
    ...
    def _check_negative_periodic(self):
        for rec in self:
            if rec.rule_type == 'periodic' and rec.periodic_qty_per_period < 1:
                raise ValidationError(_('You cannot define a negative or null number of counts per period.'))
