...
class mercadolibre_category_import(models.TransientModel):
    ...
    meli_charts = fields.Boolean(string="Importar Guias",
                                 help="Importar las guias de esta categoria, complete el genero y la marca",
                                 default=False)
    ...
    meli_category_id = fields.Char(string="MercadoLibre Category ID", help="MercadoLibre Category ID (ML????????)",
                                   default=_get_default_meli_category_id)
    meli_category_id_sel = fields.Many2one("mercadolibre.category", string="MercadoLibre Category",
                                           help="MercadoLibre Category")
    meli_recursive_import = fields.Boolean(string="Recursive Import", help="Importar todas las subramas",
                                           default=_get_default_meli_recursive_import)
    ...
    def meli_category_import(self, context=None):
        ...
        if (self.meli_category_id_sel and self.meli_charts):
            ...
            rjson_charts = self.meli_category_id_sel.get_search_chart(meli=meli, brand=self.meli_brand,
                                                                      gender=self.meli_gender)
            ...
            if (self.meli_category_id):
                ...
                catid = self.env["mercadolibre.category"].import_all_categories(self.meli_category_id,
                                                                                self.meli_recursive_import)
            else:
                ...
                for ml_cat_id in mlcat_ids:
                    ...
                    if ml_cat:
                        ...
                        catid = self.env["mercadolibre.category"].import_all_categories(meli_category_id,
                                                                                        self.meli_recursive_import)
...
class mercadolibre_category_attribute(models.Model):
    ...
    product_attributes = fields.One2many("product.attribute", "meli_default_id_attribute",
                                         string="Atributos de odoo asociados")
...
class mercadolibre_category(models.Model):
    ...
    def meli_get_category(self, category_id, meli=None, create_missing_website=False, config=None):
        ...
        if not ml_cat:
            ml_cat = self.import_category(category_id=category_id, meli=meli,
                                          create_missing_website=create_missing_website)
        ...
        if not www_cat_id and ml_cat_id:
            ...
            www_cat_id = self.create_ecommerce_category(category_id=category_id, meli=meli,
                                                        create_missing_website=create_missing_website)
    ...
    def _get_category_url(self, meli=None):
        _logger.debug("Compute _get_category_url method start")
        company = self.env.user.company_id
        ...
        for category in self:
            if (category and category.meli_category_id):
                ...
                response_cat = meli.get("/categories/" + str(category.meli_category_id),
                                        {'access_token': meli.access_token})
                rjson_cat = response_cat.json()
                # if not meli then response is different and then
                # it's accessing rjson_cat["path_from_root"][0]["id"] so arising errors
                # there is two type are reviewd
                #user not found or invalid access token
                if 'status' in rjson_cat and rjson_cat['status'] == 401:
                    raise ValidationError(rjson_cat['message'].split(':')[-1].strip('"}'))
                #*#
                ...
                category.meli_category_attributes = "https://api.mercadolibre.com/categories/" + str(
                    category.meli_category_id) + "/attributes"
                ...
                if (len(rjson_cat["path_from_root"]) >= 2):
                    ...
                    category.meli_father_category_id = rjson_cat["path_from_root"][0]["id"]
        _logger.debug("Compute _get_category_url method start")
    ...
    def _get_attributes(self, meli=None):
        _logger.info("Compute _get_attributes method start")
        ...
        # custom changes https://app.clickup.com/t/86drr13tu?comment=90170022792717
        # Solve the error while upload the product to the meli.
        for category in self:
            if (category.meli_category_id
                ...
                category.meli_category_attributes = "https://api.mercadolibre.com/categories/" + str(
                    category.meli_category_id) + "/attributes"
                resp = meli.get("/categories/" + str(category.meli_category_id) + "/attributes",
                                {'access_token': meli.access_token})
                rjs = resp.json()
                att_ids = []
                for att in rjs:
                ...
                rjson_cat = response_cat.json()
                category.is_branch = ("children_categories" in rjson_cat and len(rjson_cat["children_categories"]) > 0)
        _logger.info("Compute _get_attributes method end")
    ...
    def import_category(self, category_id, meli=None, create_missing_website=False):
        ...
        if (category_id):
            ...
            if not ml_cat_id:
                ...
                cat_fields["catalog_domain"] = "settings" in rjson_cat and "catalog_domain" in rjson_cat["settings"] and \
                                               rjson_cat["settings"]["catalog_domain"]
                cat_fields["data_json"] = json.dumps(rjson_cat)
                cat_fields["catalog_domain_json"] = self.get_catalog_domain_json(
                    catalog_domain=cat_fields["catalog_domain"], meli=meli)
    ...
    def import_all_categories(self, category_root, recursive_import=False, meli=None):
        ...
        if (category_root):
            ...
            if (rjson and "name" in rjson):
                ...
                warningobj.info(title='MELI WARNING',
                                message="Preparando importación de todas las categorías en " + str(category_root),
    ...
    def _catalog_domain_link(self):
        for cat in self:
            cat.catalog_domain_link = (cat.catalog_domain and "https://api.mercadolibre.com/catalog_domains/" + str(
                cat.catalog_domain)) or ""
    ...
    @api.depends('catalog_domain_link', 'catalog_domain_json', 'catalog_domain_chart_active',
                 'catalog_domain_chart_result')
    ...
    catalog_domain_chart_ids = fields.Many2many(comodel_name='mercadolibre.grid.chart',
                                                compute='_compute_catalog_domain_chart_ids')
        ...
        for row in self.rows:
            ...
            for attval in row.attribute_values:
                ...
                if (attval.name and (really_compare(value, attval.value) or really_compare(value, attval.name))):
                    ...
                    _logger.info("search_row_id: ret_row_id FINAL for Value: " + str(value) + " GRID Row Val: " + str(
                        ret_size_val) + " from Col Name: " + str(ret_col_name) + " Col Value: " + str(
                        ret_col_value) + " ROW ID >>> " + str(ret_row_id))
