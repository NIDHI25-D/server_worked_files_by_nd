...
def UpdateProductType( product ):
    ...
    for prod in product:
        ...
        if (prod and "type" in prod._fields and prod.type not in ['consu']):
            failed = False
            try:
                prod.write( { 'type': 'consu' } )
                _logger.info(f"Change type___________{prod.name}")
            except Exception as e:
...
def stock_inventory_action_done( self, product, stock, config ):
    ...
    stock_inventory_fields = get_inventory_fields( product, whid, quantity=stock )
...
def get_delivery_line(sorder):
    ...
    try:
        ...
        delivery_lines = sorder.order_line.filtered(lambda x: x.is_delivery and x.order_id in sorder.ids)
        #*#
        if delivery_lines:
