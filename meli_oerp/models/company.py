...
class res_company(models.Model):
    ...
    def _get_ML_sites(self,meli=False):
        ...
        if (response):
            ...
            for site in sites:
                ...
                _key_ = type(site) == dict and "default_currency_id" in site and site["default_currency_id"]  #Added by Gustavo
                if (_key_!="USD"):
                    ML_sites[_key_] = site
    ...
    def get_meli_state( self ):
        _logger.debug("Compute get_meli_state method start")
        ...
        _logger.debug("Compute get_meli_state method end")
    ...
    # custom Changes
    mercadolibre_vendor_store_ids = fields.One2many('mercadolibre.store', 'mercadolibre_company_id',
                                                    string="Mercado Libre stores")
    mercadolibre_vendor_user_type = fields.Char("ML User Type")
    #*#
            ...
            for obj in product_ids:
                try:
                    ...
                    if ( updating or creating):
                        ...
                        #we have a message
                            # custom changes https://app.clickup.com/t/865cf9d88
                            # Formate the code and replace the method that not exists in the version16.
                            # warning = self.env["meli.warning"].browse(res["res_id"])
                            if 'res_id' in res:
                                warning = self.env["meli.warning"].browse(res.get("res_id"))
                                #*#
                                if (warning):
    ...
    def meli_notifications(self, data=False):
        ...
        notifications = self.env['mercadolibre.notification'].with_user(SUPERUSER_ID)
        #*#
    ...
    def update_store(self):
        """
            ...
            Date: 01/06/23
            Task: Migration(https://app.clickup.com/t/865cf9d88)
            Purpose: Update store button not shown in the for update the put the method from the version 14.
        """
        if self.mercadolibre_seller_id:
            ml_vendor_id = self.mercadolibre_seller_id
            company = self.env.user.company_id
            # if not meli:
            meli = self.env['meli.util'].get_new_instance(company)
            vendor_info = meli.get('/users/' + ml_vendor_id, {'access_token': meli.access_token})
            vendor_info = vendor_info.json()
            # Production for get brands 171738392 other wises' ml_vendor_id
            if vendor_info.get('id'):
                self.write({'mercadolibre_vendor_user_type': vendor_info.get('user_type')})
                # res.mercadolibre_vendor_user_type = vendor_info.get('user_type')
                if vendor_info.get('user_type') == "brand":
                    self.env['mercadolibre.store'].sudo().search(
                        [('mercadolibre_company_id.id', '=', self.env.company.id)]).unlink()
                    vendors_brand_info = meli.get('/users/' + ml_vendor_id + '/brands',
                                                  {'access_token': meli.access_token})
                    vendors_brand_info = vendors_brand_info.json()
                    store_ids = []
                    for brand in vendors_brand_info.get('brands'):
                        store_ids.append((0, 0, {
                            'name': brand.get('name'),
                            'store_id': brand.get('official_store_id'),
                            'mercadolibre_company_id': self.env.company.id
                        }))
                    self.write({'mercadolibre_vendor_store_ids': store_ids})
                    # self.mercadolibre_vendor_store_ids = store_ids
                else:
                    self.env['mercadolibre.store'].sudo().search(
                        [('mercadolibre_company_id.id', '=', self.env.company.id)]).unlink()
        else:
