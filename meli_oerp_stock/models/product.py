...
from ...meli_oerp.models.versions import *
...
class product_product(models.Model):
    ...
    def _meli_get_location_id(self, meli_id=None, meli=False, config=None):
        ...
        # Custom changes multi warehouse publish product Custom
        #https://app.clickup.com/t/865d9mvqa
        # Done with requirement related to the export multiple location stock to the meli oerp.
        if company.mercadolibre_stock_pick_type == 'specific':
            loc_id = self.env["stock.location"].search(
                [('mercadolibre_active', '=', True), ('company_id', '=', company.id),
                 ('id', '=', company.mercadolibre_stock_location_to_post.id)])
        else:
            loc_id = company.meli_warehouse_ids.mapped('location_ids')
        #*#
        ...
        multi_stock_locations = ("publish_stock_locations" in config._fields)
        #*#
        ...
        if (meli_shipping_logistic_type == "fulfillment"):
            ...
            if (config.mercadolibre_stock_warehouse and not loc_id):
                loc_id = config.mercadolibre_stock_warehouse.lot_stock_idz
        ...
        for loc in loc_id:
            ...
            if (not qty_method or qty_method == 'virtual'):
                ...
                if (qty_method == 'qty_reserved'):
                    ...
                    quants = self.env['stock.quant']._gather(product_id, location_id=loc)
                    qty_available_op = (quants and sum(
                        [(quant.quantity - quant.reserved_quantity) for quant in quants])) or 0
                    # qty_available_op = product_id.get_theoretical_quantity( product_id.id, loc.id )
